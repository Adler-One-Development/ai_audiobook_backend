...
// Helpers
async function callFunction(name: string, method: string, token: string | null, body: any = null) {
...
Deno.test("MFA Flow Test", async (t) => {
  ...
  await t.step("Enroll MFA Start", async () => {
    const result = await callFunction("mfaEnrollStart", "POST", accessToken, {});
  ...
  await t.step("Enroll MFA Complete", async () => {
    ...
    const result = await callFunction("mfaEnrollComplete", "POST", accessToken, {
      factorId,
      code,
    });
