...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!project_id || !chapter_id) {
      return errorResponse("Missing parameter: project_id or chapter_id", 400);
    }
    ...
    const elevenLabsUrl = `https://api.elevenlabs.io/v1/studio/projects/${studioId}/chapters/${chapter_id}/snapshots`;
    ...
    if (!elResponse.ok) {
        ...
        // Pass through 404 if chapter not found, or other errors
        if (elResponse.status === 404) {
             return errorResponse("Chapter snapshots not found or access denied (ElevenLabs)", 404);
        }
        return errorResponse(`ElevenLabs API Error: ${elResponse.statusText}`, elResponse.status);
    }

    const elData = await elResponse.json();
    const snapshots: ElevenLabsSnapshot[] = elData.snapshots || [];

    if (snapshots.length === 0) {
        return successResponse({
            status: "success",
            message: "Chapter needs to be converted",
            snapshots: []
        });
    }
